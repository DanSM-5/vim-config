...
func! s:SetConfigurationsBefore () abort
  ...
  " Using built-in gt/gT | :h gt | :h gT
  "
...
func! s:Windows_conf_before () abort
  ...
  if filereadable(glob('~/AppData/Local/Python/bin/python.exe'))
    let g:python3_host_prog = expand('~/AppData/Local/Python/bin/python.exe')
  else
    let g:python3_host_prog = expand(glob('~/AppData/local/Programs/Python/Python3*/python.exe'))
  endif
endf
...
func! s:DefineCommands () abort
  ...
  " Tab config toggle
  command! -nargs=? -bar -bang SetTab call g:SetTab(<q-args>) | if (<bang>0) | retab | endif
