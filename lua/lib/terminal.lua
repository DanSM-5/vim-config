...
local function get_cmd(opts)
  ...
  local utils = vim.fs.joinpath(vim.fn.stdpath('config'), 'utils')
  ...
  if is_win then
    cmd = {
      ...
      'Bypass',
      '-File',
      vim.fs.joinpath(utils, 'run.ps1'),
    }
  else
    cmd = { vim.fs.joinpath(utils, 'run.sh') }
  end
...
local function call_float(opts)
  ...
  local term_autocmd_group = vim.api.nvim_create_augroup(('%s_%d_%d'):format(terminal_const.float_name, win, buf), { clear = true })
